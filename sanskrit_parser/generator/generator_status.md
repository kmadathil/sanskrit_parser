...
**Last implemented:** **Phase X (audit) + DE-UI** (`samasa_completion_plan.md`) — completing the samāsa-completion plan. **Phase X:** 2.2.30 उपसर्जनं पूर्वम् was already realized by the M2 step (D2); the rest (2.2.27/केशाकेशि, 2.2.35–37) stay deferred on machinery OUTSIDE the four mechanisms (6.3.137 unimplemented; the bahuvrīhi word-order needs the reorder to sequence before the B0 referent-gender assignment). **DE-UI:** CLI `--dvandva`/`--samahara`/`--ekasesa` (`prepare_dvandva`/`prepare_ekasesa`) + `/api/karaka` `samasa_type`/`samahara` (`_apply_dvandva`/`_apply_ekasesa`) + a `karaka.html` dvandva/ekaśeṣa dropdown; CLI+API smoke tests green (धवखदिरौ/पाणिपादम्/रामौ). **The samāsa-completion plan (D0–D3, E0–E1, S0–S2, A0–A2, X, DE-UI) is COMPLETE — all four new mechanisms M1–M4 built.**

**Next to be implemented:** The deferred tails, when needed — **6.3.137 अन्येषामपि दृश्यते** (unblocks केशाकेशि/2.2.27), the **cross-compound ṣatva** lever generalized (unblocks अग्नीषोमौ/युधिष्ठिरः/मातृष्वसा), the **bahuvrīhi word-order** reorder-before-referent sequencing (2.2.35–37), and the lexical tails (S0 5.4.74–85/6.3.97/98, S1 ~21 nipātanas, S2 5.4.69/70, gotra 1.2.65/66). Bahuvrīhi/tatpuruṣa latent gaps (1.2.48 `?pum_abs`, general cross-member ṇatva चोरभयेण) unchanged.
...
| 812 | 2.4.26 | परवल्लिङ्गं द्वन्द्वतत्पुरुषयोः | Tatpuruṣa (**T0**, saṁjñā, `bahiranga: -1`): the compound takes the uttara's gender. Already realized by `join_objects` (merge prefers the last member's liṅga, `paninian_object.py` ~L154), so this is a documenting ?paravalliNga marker that the T-liṅga exceptions (2.4.19/29/30/31) will override. **Widened for Dvandva (D0):** rp is now `[or, ?tatpuruza, ?dvandva]` (the sūtra names both) → the itaretara dvandva declines paravalliṅga too (रामकृष्णौ takes कृष्ण's masc); the SK813/2.4.27 पूर्ववदश्ववडवौ override is the dvandva-side exception |
...
| 701 | 2.1.39 | स्तोकान्तिकदूरार्थकृच्छ्राणि क्तेन | Tatpuruṣa (**T1**): a स्तोक/अन्तिक/दूर/कृच्छ्र-class pañcamī pūrva (?stoka_gaRa) + a ?kta uttara → **स्तोकान्मुक्तः** (the aluk form, via A0's 6.3.2 पञ्चम्याः स्तोकादिभ्यः — the pañcamī is now RETAINED, not luk'd) |
...
| 901 | 2.2.29 | चार्थे द्वन्द्वः | Dvandva (**D0**, `bahiranga: -1`): coordinates two-or-more prathamānta padas in the "and" sense. Gated `?dvandva_vivakza` (like `?bahuvrIhi_vivakza` — no lexical discriminator). pūrva → `?samAsaPurva` (+`?upasarjana` via 1.2.43), uttara → `?samAsa` + `?dvandva`. lp guard is `?!samAsaPurva` (NOT `?!samAsa`) so a chain's middle member keeps compounding. Declines paravalliṅga (2.4.26, widened) in the LAST member's gender. See `samasa_completion_plan.md` |
| — | 1.4.22 | द्व्येकयोर्द्विवचनैकवचने | Dvandva (**D0** derived vacana, `bahiranga: -1`, scoped to the dvandva window): both window members singular → the itaretara compound is DUAL. Sets uttara `-vacana_1 +vacana_2 +swap_viBakti`; the existing `_swap_sups` replaces the uttara's su with au. NO engine change. → धवखदिरौ |
| — | 1.4.21 | बहुषु बहुवचनम् | Dvandva (**D0** derived vacana, `bahiranga: -1`): the pūrva side already dual/plural (`lp ?!vacana_1`) → the whole is PLURAL (`+vacana_3 +swap_viBakti`). n-ary is free via the adjacent-pair window walk: राम\|कृष्ण→dual, then कृष्ण\|गोविन्द→plural → धवखदिरपलाशाः |
| 906 | 2.4.2 | द्वन्द्वश्च प्राणितूर्यसेनाङ्गानाम् | Dvandva (**D1** samāhāra, saṁjñā, `bahiranga: -1`): a प्राण्यङ्ग/तूर्य/सेनाङ्ग (`?prANyaNga`\|`?tUrya`\|`?senANga`) dvandva is ONLY समाहार (एकवत्) → sets `+samAhAra`, forces ekavacana, `overrides: [1.4.21, 1.4.22]`. 2.4.17 स नपुंसकम् then makes it neuter → पाणिपादम्. Class tag composer-supplied |
| 912 | 2.4.8 | क्षुद्रजन्तवः | Dvandva (**D1** samāhāra): a small-creature (`?kzudrajantu`) dvandva → samāhāra napum sg (यूकालिक्षम्). Same mechanism as 2.4.2 |
| 913 | 2.4.9 | येषां च विरोधः शाश्वतिकः | Dvandva (**D1** samāhāra): a natural-enemies (`?nityaviroDa`) dvandva → samāhāra napum sg (अहिनकुलम्). Same mechanism |
| 916 | 2.4.12 | विभाषा वृक्षमृगतृणधान्य… | Dvandva (**D1** samāhāra, **vibhāṣā** `optional: true`): a वृक्ष/मृग/तृण/… (`?vfkzAdi`) dvandva is OPTIONALLY samāhāra → the pre-pass forks प्लक्षन्यग्रोधम् (samāhāra napum sg) / प्लक्षन्यग्रोधाः (itaretara plural — jāti members supplied plural). aśvavaḍava/pūrvāpara/adharottara arms via the same tag |
| 821 | 2.4.17 | स नपुंसकम् | Dvandva (**D1**, saṁjñā, `bahiranga: -1`): a समाहार is NAPUṀSAKA. Fires on any `?dvandva ?samAhAra` (whether set by 2.4.2 ff. or composer-declared, the समाहार sense of चार्थ) → `+samasa_napum` (→ ?napum + gender lock via `_commit_samasa_napum`). Gated `?dvandva` so the dvigu (2.4.1) and avyayībhāva (2.4.18) napum paths are untouched |
| 930 | 5.4.106 | द्वन्द्वाच्चुदषहान्तात्समाहारे | Dvandva (**D1** samāsānta, `bahiranga: -1`): a चु/द/ष/ह-final (`?cudazaha_anta`) समाहार dvandva takes टच् (`+samasanta_TaC` → `_insert_samasanta`) → **वाक्त्वचम्** (वाच्+त्वच्+अ; वाच्→वाक् 8.2.30/8.4.55, napum nom sg अम् 7.1.24). समाहारे किम् — an itaretara dvandva takes no ṭac (the `?samAhAra` gate) |
| 813 | 2.4.27 | पूर्ववदश्ववडवौ | Dvandva (**D1** gender exception, saṁjñā, `bahiranga: -1`): अश्व+वडवा (`?aSvavaqava`) takes the PŪRVA's gender (masc, अश्व), NOT paravalliṅga → **अश्ववडवौ** (masc dual, not fem अश्ववडवे). Pre-pass override: pūrva `+pum -strI -napum +samasa_liNga_locked`; also clears the uttara's `orp: [-Ap, -strI]` (else वडवा's stale ṭāp drives the fem ā-stem — the same ?Ap trap the bahuvrīhi ādeśas hit). Moved here from the tatpuruṣa deferred group |
| 903 | 2.2.32 | द्वन्द्वे घि | Dvandva (**D2** pūrva-nipāta, `bahiranga: -1`, `purvanipata: true`): in a dvandva the घि-word (short i/u-final, per 1.4.7) goes first → हर+हरि (reversed) = **हरिहरौ**. SWEEP 1 of the pre-pass tags the uttara `?pUrvanipAta` (via `$$rp_is_purva_ghi`, which re-derives ghi — `?Gi` is a bahiranga:0 saṁjñā, unset in the pre-pass) and `_commit_purvanipata` swaps the member-units. **The 2.2.30 engine step (M2)** |
| 904 | 2.2.33 | अजाद्यदन्तम् | Dvandva (**D2** pūrva-nipāta): a vowel-initial AND a-final member goes first → कृष्ण+ईश (reversed) = **ईशकृष्णौ** (`$$rp_is_purva_ajady`). Weaker than 2.2.32, stronger than 2.2.34 (aps-order priority in the sweep) |
| 905 | 2.2.34 | अल्पाच्तरम् | Dvandva (**D2** pūrva-nipāta): the member with FEWER vowels (अच्) goes first → केशव+शिव (reversed) = **शिवकेशवौ** (`$$rp_is_purva_alpac`; शिव=2 ac, केशव=3). The most general pūrva-nipāta (the default) |
| 921 | 6.3.25 | आनङ् ऋतो द्वन्द्वे | Dvandva (**D3** ādeśa, `bahiranga: -1`): a विद्या/योनि-सम्बन्ध ऋ-final pūrva (`?vidyAyoni` + `$$is_ftanta`) in a dvandva takes आनङ् (its ऋ → आ, `xform l: A`) → **मातापितरौ, होतापोतारौ, पितापुत्रौ**. Pre-pass pūrva-substitution; the uttara keeps its own declension (होतृ→होतारौ vṛddhi, पितृ→पितरौ guṇa) |
| 922 | 6.3.26 | देवताद्वन्द्वे च | Dvandva (**D3** ādeśa): a devatā (`?devatA`) dvandva's pūrva final → आ (not only ऋ-final: मित्र a-stem → मित्रा) → **मित्रावरुणौ**. Guarded `=!agni =!div` (its apavādas 6.3.27/6.3.29 win) |
| 923 | 6.3.27 | ईदग्नेः सोमवरुणयोः | Dvandva (**D3** ādeśa, `overrides: 6.3.26`): अग्नि's इ → ई before सोम/वरुण → **अग्नीवरुणौ**. (The सोम arm अग्नीषोमौ additionally needs 8.3.82 ṣatva — see Skipped) |
| 926 | 6.3.29 | दिवो द्यावा | Dvandva (**D3** ādeśa, `overrides: 6.3.26`): whole-stem दिव् → द्यावा in a devatā-dvandva → the uttara पृथिवी (nadī) declines → **द्यावापृथिव्यौ** |
| 188 | 1.2.64 | सरूपाणामेकशेष एकविभक्तौ | Ekaśeṣa (**E0**, `bahiranga: -1`): several SAME-FORM (`$$sarUpa`, lp≡rp) padas in one vibhakti collapse to ONE. Tags the pūrva `?ekaSeza_lupta`, the uttara `?ekaSeza_Sizyate`; `_commit_ekasesa` (post-fixpoint, M3) physically deletes the लुप्त members + their sups; the survivor declines alone. Gated `?ekaSeza_vivakza`. Vacana DERIVED by the **widened 1.4.22/1.4.21** (rp gate now `[or, ?dvandva, ?ekaSeza_Sizyate]`) → **रामौ** (राम+राम), **रामाः** (×3), रामाभ्याम् (ins du). Not a compound — `_nest_samasa_members` leaves the single survivor flat. See `samasa_completion_plan.md` |
| 933 | 1.2.67 | पुमान् स्त्रिया | Ekaśeṣa (**E1**): a MASCULINE survives over its same-base feminine (`?tallakzaRa` — tallakṣaṇa/bhāṣitapuṁska, हंसी=हंस+ṅīp) → **हंसौ**. Same update as 1.2.64 (survivor = rp). The `?tallakzaRa` gate is criterial — without it 1.2.67 wrongly collapses भ्रातृ+स्वसृ / मातृ+पितृ (different lexemes, handled by 1.2.68/1.2.70) |
| 934 | 1.2.68 | भ्रातृपुत्रौ स्वसृदुहितृभ्याम् | Ekaśeṣa (**E1**): भ्रातृ survives over स्वसृ, पुत्र over दुहितृ (different lexemes, specially licensed) → **भ्रातरौ, पुत्रौ** |
| 936 | 1.2.70 | पिता मात्रा | Ekaśeṣa (**E1**): पितृ survives over मातृ → **पितरौ** (the ekaśeṣa arm; the dvandva alternative मातापितरौ is D3's 6.3.25, a different intent) |
| 938 | 1.2.72 | त्यदादीनि सर्वैर्नित्यम् | Ekaśeṣa (**E1**): a त्यद्-ādi (`?tyadAdi` pronoun) survives over any other word → राम+तद् = **तौ** |
| 939 | 1.2.73 | ग्राम्यपशुसङ्घेष्वतरुणेषु स्त्री | Ekaśeṣa (**E1**, `overrides: 1.2.67`): for domestic animals (`?grAmyapaSu`) the FEMININE survives (apavāda to 1.2.67) → अज+अजा = **अजे**. The override is exercised even when both tallakṣaṇa + grāmyapaśu apply |
| 946 | 5.4.78 | ब्रह्महस्तिभ्यां वर्चसः | Sarvasamāsānta (**S0**, `bahiranga: -1`): a वर्चस्-final tatpuruṣa after ब्रह्मन्/हस्तिन् takes अच् (`?samasanta_TaC`) → **ब्रह्मवर्चसम्** (ṣaṣṭhī; वर्चस्+अ → a-stem, napum am by paravalliṅga 2.4.26). **Fixed a latent `wac` bug:** `wac` carried an intrinsic `?pum` (a pre-2.4.29 shortcut for the द्व्यह्न manual test stem) that mis-gendered every napum wac samāsānta; removed it — `wac` is now gender-neutral, and द्व्यह्न sets pum on its local wac copy (representing 2.4.29). See `samasa_completion_plan.md` |
| 947 | 5.4.79 | अवसमन्धेभ्यस्तमसः | Sarvasamāsānta (**S0**): a तमस्-final compound after अन्ध (karmadhāraya) takes अच् → **अन्धतमसम्** (napum by paravalliṅga). The अव/सम् (avyayībhāva) arms deferred |
| 949 | 5.4.81 | अन्ववतप्ताद्रहसः | Sarvasamāsānta (**S0**): a रहस्-final compound after तप्त (karmadhāraya) takes अच् → **तप्तरहसम्** (napum by paravalliṅga). The अनु/अव arms deferred |
| 945 | 5.4.77 | अचतुर…गोष्ठश्वाः (nipātana gaṇa) | Sarvasamāsānta (**S1**, `bahiranga: -1`): the 25-item nipātana list — ONE rule gated on a composer gaṇa tag `?nipAta_5477` on the uttara → अच् (`?samasanta_TaC`), across compound types. Representative members: the उक्षन् karmadhāraya trio **जातोक्षः / महोक्षः** (महत्→महा 6.3.46) **/ वृद्धोक्षः**, and the dvandva **वाङ्मनसे** (वाच्+मनस्; वाग्मनसे optional-anunāsika variant). Rest deferred — see Skipped |
| 956 | 5.4.71 | नञस्तत्पुरुषात् | Sarvasamāsānta prohibition (**S2**, `bahiranga: -1`, `overrides: 5.4.91`): a नञ्-tatpuruṣa takes NO samāsānta → sets `?samasanta_niziDDa` on the uttara (the guard 5.4.91 now honours) → न राजा = **अराजा** (राजन् keeps its declension; 5.4.91's ṭac, which gave *अराजः, is blocked). पथिन् excluded (`?!paTin` — 5.4.72's domain). परमराजः (non-नञ्) still takes ṭac |
| 957 | 5.4.72 | पथो विभाषा | Sarvasamāsānta prohibition (**S2**, `optional: true`): a नञ्पूर्व पथिन् OPTIONALLY takes अच् → **अपथम्** (with अच् → अपथ, napum by 2.4.30) / **अपन्थाः** (without → पथिन् declension पन्थाः). Vibhāṣā fork |
| 815 | 2.4.30 | अपथं नपुंसकम् | Tatpuruṣa T-liṅga (**now landed via S2**, `bahiranga: -1`): a पथिन्-final tatpuruṣa that took the अच् (5.4.72) is NAPUṀSAKA → अपथम् (`?samasanta_TaC`-gated `+samasa_napum`; the no-अच् fork stays masc अपन्थाः). Clears a standing deferral |
| 958 | 6.3.1 | अलुगुत्तरपदे | Aluk adhikāra (**A0**, M4). ALUK: the pūrva's vigraha sup is normally luk'd (2.4.71); the aluk rules SUPPRESS that by tagging the pūrva `?aluk`, and **2.4.71 gained a `?!aluk` guard** so the case-ending survives INTO the compound (a full pada → pada-junction sandhi). Folded into each aluk rule as `?aluk`. See `samasa_completion_plan.md` |
| 959 | 6.3.2 | पञ्चम्याः स्तोकादिभ्यः | Aluk (**A0**, `bahiranga: -1`): the pañcamī of स्तोक/अन्तिक/दूर/कृच्छ्र has aluk → **स्तोकान्मुक्तः** (स्तोकात्+मुक्त, त्→न् 8.4.45; स्तोकाद्मुक्तः jaśtva variant). n-final junction. **Flips the old स्तोकमुक्तः deferral** |
| 960 | 6.3.3 | ओजःसहोऽम्भस्तमसस्तृतीयायाः | Aluk (**A0**): tṛtīyā of ओजस्/सहस्/अम्भस्/तमस् → **ओजसाकृतम्** (s-stem instrumental ओजसा retained) |
| 964 | 6.3.7 | वैयाकरणाख्यायां चतुर्थ्याः | Aluk (**A0**): caturthī of आत्मन् → **आत्मनेपदम्** (e-final; the grammarians' term — the generator's own metalanguage) |
| 971 | 6.3.13 | बन्धे च विभाषा | Aluk (**A1**, `optional: true`): a saptamī before बन्ध OPTIONALLY has aluk → **हस्तेबन्धः / हस्तबन्धः** (forms via 2.1.41's बन्ध arm) |
| 979 | 6.3.21 | षष्ठ्या आक्रोशे | Aluk (**A2**): a ṣaṣṭhī in the आक्रोश (`?AkroSa`) sense is retained → **चौरस्यकुलम्**; non-ākrośa चौरकुलम् (negative control, the ṣaṣṭhī luks) |
...
| 654 | 2.2.30 | उपसर्जनं पूर्वम् | **IMPLEMENTED (Dvandva D2)** — the physical pūrva-nipāta engine step now exists: `_purvanipata_sweep` (SWEEP 1 of the samāsa pre-pass) + `_commit_purvanipata` physically move the member a `purvanipata: true` rule tags `?pUrvanipAta`. Realized for the dvandva ordering rules 2.2.32/33/34 (हरिहरौ/ईशकृष्णौ/शिवकेशवौ from reversed input). The bahuvrīhi 2.2.35–37 group + 2.2.27 sarūpa can now use the same step (phase X). n-ary reorder is a bubble pass (approximate; exact for 2-member) | — |
| 902 | 2.2.31 | राजदन्तादिषु परम् | Deferred (Dvandva D2) — a lexical **ākṛtigaṇa** (राजदन्तादि) where the normally-first member goes LAST. Spans compound TYPES (राजदन्त = दन्तानां राजा is a ṣaṣṭhī tatpuruṣa, not a dvandva), so it is not dvandva-scoped; needs the gaṇa membership + per-pair order data. The M2 reorder mechanism (2.2.30, now done) will carry it once the gaṇa is added | राजदन्तः, अर्धधर्मौ … |
...
| 966–984 (part) | 6.3.4–6, 6.3.9–12, 6.3.14–20, 6.3.22–24, 8.3.84, 8.3.85, 8.3.95 | मनसः… / हलदन्तात्सप्तम्याः / गवियुधिभ्यां स्थिरः / मध्याद्गुरौ / स्वाङ्गात् / कृति बहुलम् / …जे / शय-वास / पुत्रे / ऋतो विद्यायोनि / स्वसृपत्योः / मातृ-पितृ ṣatva | Deferred (Aluk A1/A2) — the M4 mechanism is landed (A0/A1/A2), these are the remaining aluk cases needing OTHER machinery: **ṣatva** (8.3.95 युधिष्ठिरः/गविष्ठिरः, 8.3.84/85 मातृष्वसा/मातुःष्वसा — cross-junction ष, same class as the deferred D3 8.3.82); **vyadhikaraṇa-bahuvrīhi** word-order (6.3.9/12 त्वचिसारः/कण्ठेकालः — need 2.2.35, phase X); **kṛd-anta upapada** (6.3.14/15/16 स्तम्बेरमः/दिविजः); the **sense-gaṇa tail** (6.3.4/5/6/17/18/22/23/24). The **prohibitions 6.3.19/20** (सांकाश्यसिद्धः, समस्थः) are **vacuous by default** — 2.4.71 already luks | युधिष्ठिरः, मातृष्वसा, कण्ठेकालः, स्तम्बेरमः, दिविजः |
...
| 822 | 2.4.19, 2.4.31 | तत्पुरुषोऽनञ्कर्मधारयः / अर्धर्चाः पुंसि च | Deferred (tatpuruṣa T-liṅga) — 2.4.29 रात्राह्नाहाः पुंसि (ahan-family → masc) and **2.4.30 अपथं नपुंसकम् (अपथम्, now landed via S2/5.4.72)** implemented. 2.4.19 (napuṁsaka only in the 2.4.17 saṃjñā-doubtful domain — inert without saṃjñā tagging, would wrongly napuṁsaka-ise ordinary masc tatpuruṣas like राजपुरुषः); 2.4.31 needs the अर्धर्चादि अच् (5.4.74 + अर्ध/ऋच् stems) | अर्धर्चः/अर्धर्चम् … |
| 954/955 | 5.4.69, 5.4.70 | न पूजनात् / किमः क्षेपे | Deferred (Sarvasamāsānta S2) — the same `?samasanta_niziDDa` block as the landed 5.4.71/72, on a different pūrva-class: 5.4.69 सुराजा/अतिराजा needs the सु/अति **pūjana** prefix compound (+ the स्वतिभ्याम् vārttika so परमराजः is untouched); 5.4.70 किंराजा needs the किम् compound + a `?kzepa` sense. The mechanism is proven | सुराजा, अतिराजा, किंराजा |
...
| 813–828 | 2.4.20–25, 1.2.58–63 | संज्ञायां कन्थोशीनरेषु / उपज्ञोपक्रमम् / छाया बाहुल्ये / सभा राजामनुष्यपूर्वा / अशाला च / विभाषा सेनासुरा…; जात्याख्यायाम्… बहुवचनम् / अस्मदो द्वयोश्च / फल्गुनीप्रोष्ठपदानाम् / तिष्यपुनर्वस्वोः | Deferred (tatpuruṣa T-liṅga) — **saṃjñā-domain gender + number** rules (kanthā/upajñā/chāyā/sabhā/aśālā/senā gender; jāti-bahuvacana, asmad-dual, nakṣatra-number). Inert without saṃjñā / nakṣatra-number tagging, same class as the deferred 2.4.19. (**2.4.27 पूर्ववदश्ववडवौ is now IMPLEMENTED** — Dvandva D1, अश्ववडवौ; removed from this row) | उपज्ञम्, राजसभम्, फल्गुनी/फल्गुन्यौ … |
| 907–920 | 2.4.3–16 (part) | अनुवादे चरणानाम् / अध्वर्युक्रतुरनपुंसकम् / अध्ययनतोऽविप्रकृष्ट… / जातिरप्राणिनाम् / विशिष्टलिङ्गो नदीदेशः / गवाश्वप्रभृतीनि / विप्रतिषिद्धम् / न दधिपयआदीनि / अधिकरणैतावत्त्वे / विभाषा समीपे | Deferred (Dvandva D1) — the **lexically/contextually gated** ekavadbhāva sūtras. Each needs a domain-specific class or context tag (anuvāda, yajurveda-kratu, adhyayana-proximity, jāti-aprāṇin, nadī-deśa different-gender, gavāśva gaṇa, vipratiṣiddha, dadhipayas-list, adhikaraṇa-number, sāmīpya). **No new engine capability** — the D1 samāhāra mechanism (2.4.2/8/9/12 + 2.4.17) covers them all the moment a class tag is added. **2.4.14 न दधिपयआदीनि is vacuous** until 2.4.6 जातिरप्राणिनाम् is implemented (nothing makes दधि+पयस् ekavat, so दधिपयसी already falls out as an itaretara dual) | अर्काश्वमेधम्, धानाशष्कुलि, गङ्गाशोणम्, तक्षायस्कारम्, गवाश्वम्, शीतोष्णम्/शीतोष्णे, दधिपयसी, उपदाशं दन्तोष्ठम् |
| 924 | 8.3.82 | अग्नेः स्तुत्स्तोमसोमाः | Deferred (Dvandva D3) — अग्नीषोमौ needs the स of सोम → ष **across the compound boundary** (after अग्नी). This is a cross-compound ṣatva, the same class as the general cross-member ṇatva gap (8.4.1 below) — it would need a `?samasta_zatva`-style lever. अग्नीवरुणौ covers 6.3.27 without it | अग्नीषोमौ, अग्निष्टोमः, अग्निष्टुत् |
| 929 | 6.3.32 | मातरपितरावुदीचाम् | Deferred (Dvandva D3) — the optional Northern मातरपितरौ (मातृ → मातर्, ऋ→अर्) leaves an **r-final pūrva** whose junction with पितृ mis-fires to a visarga (मातःपितरौ, via 8.3.15 खरवसानयोः) where मातरपितरौ wants र retained. The माता form (6.3.25) is the one that surfaces | मातरपितरौ |
| 925/928 | 6.3.28, 6.3.31 | इद्वृद्धौ / उषासोषसः | Deferred (Dvandva D3) — 6.3.28 (आग्निमारुतम्) needs the taddhita उभयपदवृद्धि of SK1239 (out of the samāsa range); 6.3.31 (उषासासूर्यम्) is a Vedic-only citation | आग्निमारुतम्, उषासासूर्यम् |
| 931/932 | 1.2.65, 1.2.66 | वृद्धो यूना / स्त्री पुंवच्च | Deferred (Ekaśeṣa E1) — the gotra/yuvan ekaśeṣa (गार्ग्यश्च गार्ग्यायणश्च = गार्ग्यौ; गार्गी…= गर्गाः) needs the gotra/yuvan taddhita derivation (यञ्/फक् + the 2.4.64 लुक्), not modelled | गार्ग्यौ, गर्गाः, दाक्षी |
| 935 | 1.2.69 | नपुंसकमनपुंसकेनैकवच्च | Deferred (Ekaśeṣa E1) — the neuter survives (शुक्लम्), but its **optional ekavat** (शुक्लम् / शुक्लानि) is a vacana mode outside the itaretara-sum model (the survivor would be singular OR plural regardless of the 2-member count) | शुक्लम्/शुक्लानि |
| 937 | 1.2.71 | श्वशुरः श्वश्वा | Deferred (Ekaśeṣa E1) — श्वशुर survives over श्वश्रू, but the attested श्वाशुरौ needs a श्वशुर→श्वाशुर vṛddhi not triggered here | श्वाशुरौ, श्वश्रूश्वाशुरौ |
| 940–953 (part) | 5.4.74, 75, 76, 80, 82–85, 6.3.97, 6.3.98 | ऋक्पूरब्धूःपथाम् / सामलोम्नः / अक्ष्णोऽदर्शनात् / श्वसो… / प्रतेरुरसः / अनुगवम् / द्विस्तावा / उपसर्गादध्वनः / अप ईत् / ऊदनोर्देशे | Deferred (Sarvasamāsānta S0) — the **same अच् (`?samasanta_TaC`) insertion** as the landed 5.4.78/79/81, on a different uttara-class/compound-type; each needs only its stem + compound formation. **5.4.76 गवाक्षः** specifically blocked on go-stem optional-sandhi over-generation (गोक्षः/गोऽक्षः, 6.1.122–124). **5.4.74** would clear the standing 2.4.30/2.4.31 (अपथम्/अर्धर्चः) once अर्ध/ऋच्/nañ-पथिन् formations land; **6.3.97/98** (द्वीपम्) are the ईत्/ऊत् ādeśas on 5.4.74's अप् | अर्धर्चः, सुपथः, प्रतिलोमम्, गवाक्षः, प्रत्युरसम्, अनुगवम्, प्राध्वः, द्वीपम् |
| 945 (part) | 5.4.77 (gaṇa tail) | अचतुर/विचतुर/सुचतुर, स्त्रीपुंसौ, धेन्वनडुहौ, ऋक्सामे, अक्षिभ्रुवम्, दारगवम्, ऊर्वष्ठीवम्, पदष्ठीवम्, नक्तन्दिवम्, रात्रिन्दिवम्, अहर्दिवम्, सरजसम्, निःश्रेयसम्, पुरुषायुषम्, द्व्यायुषम्/त्र्यायुषम्, ऋग्यजुषम्, उपशुनम्, गोष्ठश्वः | Deferred (Sarvasamāsānta S1) — the ~21 remaining 5.4.77 nipātanas. **Same nipātana अच्** (`?nipAta_5477` → `?samasanta_TaC`) as the landed जातोक्षः/महोक्षः/वृद्धोक्षः/वाङ्मनसे, on each member's own stem-pair + compound; a few carry extra nipātita changes (ṭi-lopa पदष्ठीवम्, samprasāraṇa उपशुनम्, पाद→पद्) or need irregular stems (अनडुह्, श्वन्, अष्ठीवत्) | स्त्रीपुंसौ, ऋक्सामे, अक्षिभ्रुवम्, निःश्रेयसम्, ऋग्यजुषम्, उपशुनम् |
