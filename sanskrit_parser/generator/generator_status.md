...
| Category | Count |
|---|---|
| Sutras implemented | 383 |
| Sutras skipped / deferred | 69 |
| Sutras uncatalogued / not yet planned | ~65 |
| **Total sutras accounted for** | **~517** |
...
| dvyUDnI | f | SK485 saṃkhyādi arm | dvi + UDas; SK485 peeks `llp:=dvi` → NIp overrides SK484 → द्व्यूध्नी; i+ū → yū (6.1.77 yaṇ) |
| atyUDnI | f | SK485 avyayādi arm | ati (avyaya stem) + UDas; SK485 peeks `llp:?avyaya` → NIp → अत्यूध्नी |
| dvidAmnI | f | SK486 dāman arm | dvi + dāman bahuvrīhi; SK486 peeks `llp:=dvi` → arm 1 NIp → 6.4.134 drops -an upadhā 'a' → द्विदाम्नी |
| dvihAyanI | f | SK486 hāyana arm (age sense) | dvi + hāyana (?vayasi); SK486 peeks `llp:=dvi` → arm 2 NIp → 6.4.148 drops final 'a' → द्विहायनी |
| dvihAyanA | f | SK486 hāyana negative (non-age → ṭāp) | dvi + hāyana, `llp:=dvi` but NO ?vayasi → SK486 fails → 4.1.4 ṭāp → द्विहायना (ramā-type). Vasu: द्विहायना शाला 'a hall of two years' standing'. Required 6.1.87 to strip `sarvanAmasTAna` from the post-guṇa suffix so 6.4.8 does not re-fire on the n-final surface (hāyan|e) |
| trihAyaRI | f | SK486 vārttika (tri + hāyana, age) | tri + hāyana (?vayasi); the 4.1.27.1 vārttika peeks `llp:[=tri,=catur]` → n→ṇ → SK486 (`llp:=tri`) NIp → 6.4.148 a-lopa → त्रिहायणी. 'h' intervening between r and final n of -nām blocks 8.4.2 ṇatva on suffix → gen pl त्रिहायणीनाम् |
...
| trihAyanA | f | SK486 vārttika negative (non-age tri + hāyana → ṭāp) | tri + hāyana, `llp:=tri` but NO ?vayasi → vārttika & SK486 fail → 4.1.4 ṭāp → त्रिहायना (Vasu's exact शाला example). 8.4.x ṇatva blocked by 'h' between r and stem n |
