...
| Category | Count |
|---|---|
| Sutras implemented | 361 |
| Sutras skipped / deferred | 70 |
| Sutras uncatalogued / not yet planned | ~65 |
| **Total sutras accounted for** | **~471** |
| Stems with full vibhakti test tables | 263 |
...
| 505 | 4.1.49 | इन्द्रवरुण…आनुक् | `?indrAnuk` (12 stems): **ānuk augment** आन् (`lc: lc+dirgha(l)+str("n"), l: null`) + NIz (overrides 4.1.4/4.1.5) → इन्द्राणी/रुद्राणी (ṇatva), वरुणानी/हिमानी/अरण्यानी/मातुलानी (n stays). Required the 6.4.134 `ll: "=a"` fix (short-अ only). Senses + मातुली optional-ānuk vārttika deferred |
| 6.4.134 | 6.4.134 | अल्लोपोऽनः (refinement) | `ll: a`→`ll: "=a"`: अल् names only the short अ; a savarṇa match wrongly deleted the long आ of SK505's ānuk augment (इन्द्रान्→इन्द्र्णी). All real an-stems (rājan/takṣan/ahan) have short-a penult → unaffected |
...
| 499 | 6.4.149 | सूर्यतिष्यागस्त्यमत्स्यानां य उपधायाः | Partial — taddhita-derived avayava forms deferred | Only the **non-taddhita** path is implemented: matsya is a gaurādi member, so the vārttika मत्स्यस्य ङ्याम् licenses y-lopa directly before the ṅīṣ ī → मत्सी (here `?sUryAdi` rides on the matsya stem itself; no propagation needed). Vasu's other examples सौरी/तैषी/आगस्ती are taddhita-derived (e.g. सूर्य + अण् 4.3.112 → सौर्य → ī): for 6.4.149 to treat सौर्य's upadhā य as a "सूर्य-अवयव", `?sUryAdi` must propagate from सूर्य through the aṇ-derivation onto सौर्य. That needs the 4.3.x taddhita machinery (aṇ/ṣyañ derivation + tag propagation in join_objects), not yet present — deferred |
...
| 503 | 4.1.45 | बह्वादिभ्यश्च | Partial — gaṇasūtras | The gaṇasūtra alternants (कृदिकारादक्तिनः रात्रि/रात्री; शकटि/शकटी; पद्धति/पद्धती) are deferred; bahu is the representative head |
