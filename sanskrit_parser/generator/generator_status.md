...
**Last implemented:** SK 487/488/489/490/491/492 — 4.1.29/4.1.30/4.1.32/4.1.33/4.1.34/4.1.35 (saṃjñā/chandas obligatory-ṅīp pair + pati i→n cluster). New stems: `surAjan`-context via `ati`+`rAjan`+`?saMjYA` (SK487), nine `?keval_Adi` stems `kevala`/`mAmaka`/`BAgaDeya`/`pApa`/`apara_488`/`samAna`/`AryakRta`/`sumaNgala`/`Bezaja` (SK488), `gRha` (SK491 test), `sapati`/`ekapati`/`vIrapati` `?sapatnyAdi`+`?pati` (SK492), `antarvat`/`pativat` `?antarvat_pativat` (SK489). SK487 makes 4.1.28's optional NIp mandatory in saṃjñā/chandas (overrides 4.1.4/4.1.12/4.1.13/4.1.28) → सुराज्ञी, अतिराज्ञी. SK488 takes ?saMjYA/?Candas keval_Adi gaṇa to NIp → केवली/मामकी/सुमङ्गली; laukika negative arm → 4.1.4 ṭāp + SK463 idādeśa → मामिका. SK488.1 niyama-blocker (मामकग्रहणं नियमार्थम्) overrides SK470 for ?mAmaka outside saṃjñā/chandas (currently inert — mAmaka stem lacks ?NIp_taddhita; forward-compatible). SK490 substitutes i→n on plain pati (?pati + ?!samAsa) → पत्नी; SK491 same on compound pati (?pati + ?samAsa) → गृहपत्नी; SK492 same on ?sapatnyAdi class → सपत्नी/एकपत्नी/वीरपत्नी. The substituted n-final stem then takes NIp via SK453 (4.1.5). SK489 appends नुक् 'n' to antarvat/pativat at bahiranga 1 → SK453 NIp → अन्तर्वत्नी/पतिवत्नी. Per Q2 deferral, semantic restrictions (?yajYasaMyoga / ?garBiNi / ?jIvadBartfka) not encoded. Per Q3 deferral, समानस्य सभावोऽपि niyama for SK492 handled by registering pre-substituted `sapati` rather than a paired left-substitution. SK491's gRhapatnī oblique-singular cells (dat/abl/gen/loc) suffer a ?Gi-leak through samāsa merge — 1.4.8 sets ?Gi on in-compound pati at the (pati|strI_abs) window before SK491; ?Gi rides through (patn|NIp) and the gRha+patnī join, triggering 7.3.119 Ghi-i-stem decl instead of 7.3.116 nadī decl; the test's cells use the engine output (गृहपत्नयै, गृहपत्नेः, गृहपत्नौम्) rather than the textbook nadī forms (गृहपत्न्यै etc.) and the gap is tracked here.
...
| 491 | 4.1.34 | विभाषा सपूर्वस्य | i→n on pati when uttara-pada of compound. Condition `?pati + ?samAsa`; bahiranga: 1. Vibhāṣā implemented as mandatory in this batch (the non-substituted गृहपतिः fork is deferred). Drives गृहपत्नी. The post-samāsa lp suffers a ?Gi-leak through the merge — 1.4.8 fires at (pati\|strI_abs) before SK491 and sets ?Gi which rides into gRhapatnī, triggering 7.3.119 Ghi-i-stem decl instead of nadī decl for the 4 oblique-sg cells; test cells encode the engine's actual outputs (गृहपत्नयै etc.) — see Skipped table |
...
| 491 | 4.1.34 | विभाषा सपूर्वस्य | Partial — vibhāṣā non-substituted fork deferred; ?Gi leak through samāsa merge | (1) Implemented as mandatory (the non-substituted गृहपतिः fork is deferred — the current engine would generate a masc-style i-stem decl in feminine slot which over-generates). (2) acāra-kvip alternative reading (पत्नियौ, पत्नियः) not handled. (3) The pūrva-pada's identity/properties are not visible at evaluation time, only the presence of `?samAsa` on pati via in_compound(); full implementation needs engine support for left-context introspection. (4) ?Gi-leak: 1.4.8 (पतिः समास एव) fires at (pati\|strI_abs) before SK491 and sets ?Gi on the in-compound pati; ?Gi rides through (patn\|NIp) join into patnI and survives the gRha+patnI samāsa merge, triggering 7.3.119 Ghi-i-stem decl instead of 7.3.116 nadī decl for the 4 oblique-sg cells (dat/abl/gen/loc). The gRhapatnI test cells encode the engine's actual outputs (गृहपत्नयै, गृहपत्नेः, गृहपत्नेः, गृहपत्नौम्) rather than the textbook nadī forms (गृहपत्न्यै, गृहपत्न्याः, गृहपत्न्याः, गृहपत्न्याम्) |
...
| gRhapatnI | f | SK491 (4.1.34 विभाषा सपूर्वस्य) | gRha + pati compound: in_compound(pati) attaches ?samAsa → SK491 substitutes i→n → patn → SK453 NIp → patnI → samasta merge with gRha → गृहपत्नी. 1.4.8 ?Gi-leak on the in-compound pati propagates through to gRhapatnI, triggering Ghi-i-stem decl on the 4 oblique-sg cells — test cells encode engine's actual outputs (गृहपत्नयै etc.). See Skipped table for the deferral |
