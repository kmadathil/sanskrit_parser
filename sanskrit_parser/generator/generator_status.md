...
**Last implemented:** **Bahuvrīhi Phase B0** (SK829/2.2.23 + SK830/2.2.24) — spine + core samānādhikaraṇa formation + the one new mechanism, **anyapadārtha (external-referent) gender**: a bahuvrīhi is exocentric, declining in the composer-supplied referent gender (`?referent_*` + `?samasa_liNga_locked`), not the uttara's (contrast SK812/2.4.26). Derives पीताम्बरः/पीताम्बरा/पीताम्बरम् + a masc vibhakti sweep, प्राप्तोदकः (`test_samasa_bahuvrihi.py`, suite 8161 green). Detail: SK829/830 row + `bahuvrihi_plan.md`.

**Next to be implemented:** Bahuvrīhi **B1 — puṁvadbhāva** (SK831/6.3.34 + SK836–842/6.3.35–41): fem pūrvapada → masc bhāṣitapuṁska form (दीर्घजङ्घः) + prohibitions; then B2–B4 + B-UI (session prompts in `bahuvrihi_plan.md`). Deferred in B0: physical pūrva-nipāta (2.2.30 + word order 2.2.35–37) — tagging + input-order validation only. Tatpuruṣa deferrals unchanged — see the Skipped table + `tatpuruza_plan.md`.
