...
**Next to be implemented:** Tatpuruṣa Phase T2 onward — karmadhāraya/dvigu (T2, incl. 2.1.23), nañ (T3), prādi/gati (T4), samāsānta (T5), gender exceptions (T-liṅga), composer/CLI (T-UI). See `tatpuruza_plan.md` for phasing + session prompts. **ṇatva fix (2026-07-03):** the samāsa pre-pass now nests compound members into a hierarchical sub-prakriya (`_nest_samasa_members`) so a declining compound resolves as one `samasta_pada` before meeting the sentence — the uttara ṣ/र now triggers ṇatva (राजपुरुषेण, कृष्णपुरुषेण, मासपूर्वेण), which flat tagging blocked (राजपुरुषेन). **Narrower residual:** a pūrva-only ṇatva trigger reaching the uttara's sup (चोरभयेण) is still not applied — a PRE-EXISTING engine gap (the CLI `in_compound` path shows it too), unrelated to the samāsa pre-pass. (Kāraka and strī/taddhita deferrals remain in the Skipped table.)
