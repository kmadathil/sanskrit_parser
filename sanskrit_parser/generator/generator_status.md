...
_Earlier sets — the entire kāraka-prakaraṇam (SK 532–646, phases K0–K7 + K-UI) and the
2026-06-27 kāraka review fixes — are recorded in the Implemented-Sutras tables below, in
`karaka_plan.md`, and in git history._

**Last implemented — Samāsa-prakaraṇam: tatpuruṣa (SK 684–, Phase T0)** (tatpuruṣa samāsa plan `tatpuruza_plan.md`, Phase T0 — dvitīyā-tatpuruṣa proof-of-concept). Unlike the avyayībhāva (which becomes an indeclinable → invariant अम्), a **tatpuruṣa declines normally in the uttara's gender** (2.4.26 परवल्लिङ्गम्): the uttara keeps its own sup and inflects, while the pūrva (the case-marked upasarjana) sup-luks via the existing 2.4.71 — so we do NOT tag `?avyaya`/`?napum` on the uttara. Gender inheritance is already realized by `join_objects` (the compound merge prefers the last member's gender, `paninian_object.py` ~L154), so **2.4.26 is a documenting saṁjñā** (`?paravalliNga` marker) that the T-liṅga exceptions will override. New rules (`bahiranga: -1`, reusing the samāsa pre-pass spine); the whole **dvitīyā-tatpuruṣa block SK 684–691 (2.1.22–29)**: **2.1.22 तत्पुरुषः** saṁjñā (fused into 2.1.24, as 2.1.5 was fused into 2.1.6); **2.1.24** द्वितीया श्रितातीत… — a dvitīyā noun (`?viBakti_2`) + a śrita-gaṇa uttara (`?srita_gaRa`: श्रित/अतीत/…) → कृष्णश्रितः; **2.1.25** स्वयं क्तेन (svayam + `?kta` → स्वयंकृतम्); **2.1.26** खट्वा क्षेपे (खट्वा dvitīyā + `?kta`, `?semantic_kzepa` → खट्वारूढः); **2.1.27** सामि (sāmi + `?kta` → सामिकृतम्); **2.1.28** कालाः (kāla-word `?kAlavAcaka` dvitīyā + `?kta`, non-atyanta → मासप्रमितः); **2.1.29** अत्यन्तसंयोगे च (kāla-word dvitīyā + any word, `?semantic_atyantasaMyoga` → मुहूर्तसुखम्). Each sets pūrva `?samAsaPurva`, uttara `?samAsa`+`?tatpuruza`, gated on `?samAsa_vivakza` (or a semantic sense) — "only-when-intended", not a fork. Plus **2.4.26** परवल्लिङ्गम् (documenting saṁjñā; gender inheritance is already `join_objects`' last-member-wins merge). Reused unchanged: 1.2.43 (upasarjana), 2.4.71 (pūrva sup-luk). कृष्णश्रितः gets a full a-stem masc vibhakti sweep (कृष्णश्रितम्/कृष्णश्रितेन/…/कृष्णश्रितात् — proving it inflects, unlike the avyayībhāva's invariant अम्). New pratipadikas `Srita` (`?srita_gaRa`), `kfta`/`rUQa`/`pramita` (`?kta`), `suKa`, `KawvA`, `muhUrta`+`mAsa` (`?kAlavAcaka`); `svayam`/`sAmi` already in avyaya.py. 14 cases in `test/test_samasa_tatpurusha.py`. **Deferred to later T-phases:** the other five vibhakti-tatpuruṣas (T1), karmadhāraya/dvigu (T2, incl. 2.1.23 द्विगुश्च), nañ (T3), prādi/gati (T4), samāsānta (T5), gender exceptions (T-liṅga), composer/CLI (T-UI); 2.2.30 physical reorder stays deferred (the case member is already pūrva); स्वयंकृतम्'s compound-internal म्→anusvāra (8.3.24) is a known limitation (see Skipped table).

**Samāsa-prakaraṇam: avyayībhāva (SK 647–677)** (avyayībhāva samāsa plan, S0–S3, with the S-UI composer + CLI). A new **samāsa pre-pass** (`AntarangaPrakriya._samasa_prepass`, `bahiranga: -1`) runs after the kāraka pre-pass + sup-insertion (सह सुपा 2.1.4): the 113 kāraka rules were renumbered `bahiranga: -1 → -2` to make room (lower = more antaraṅga = earlier; kāraka -2 fixes vibhakti, samāsa -1 consumes it). The pre-pass scans adjacent (pūrva | uttara) member windows and assigns the samāsa saṁjñā / member-role (`samAsaPurva`/`samAsa`) / `avyayIBAva` / `upasarjana` (1.2.43) tags; it does NOT luk internal vibhakti (main scan) or physically reorder (2.2.30 deferred — the avyaya is already pūrva). Main-scan morphology: 1.1.41 (avyaya on the uttara), 2.4.71 (pūrva sup-luk), 2.4.82 (luk) / 2.4.83 (a-stem → अम्, overrides 2.4.82; **त्वपञ्चम्याः sub-rule 2.4.83.1** → ablative उपकृष्णात्), 2.4.18 (napuṁsaka) + 1.2.47 (hrasva). Sutras: **nitya** 2.1.5/2.1.6 (समीप/विभक्ति/अत्यय/…), 2.1.7 (यथा), 2.1.8 (यावत्), 2.1.9 (सुप्प्रति, noun-pūrva), 2.1.10 (अक्षशलाका, noun-pūrva); **vibhāṣā** (≥ SK665, gated by `?samAsa_vivakza` — "only when intended", not a fork) 2.1.12/2.1.13/2.1.14 (प्रत्यग्नि)/2.1.15/2.1.16/2.1.19/2.1.20; **samāsānta/junction/bahula** the full rule-driven TaC set — 5.4.107 (शरत्→उपशरदम्), 5.4.108+6.4.144 (an→उपराजम्), 5.4.110 (nadī→उपनदम्), 5.4.111 (jhay→उपसमिधम्), 5.4.112 (giri→उपगिरम्), 5.4.109 (napuṁsaka अन्यतरस्याम् → OPTIONAL TaC, forking → उपचर्म/उपचर्मम्) — each a pre-pass rule setting ?samasanta_TaC + the generic `_insert_samasanta` step; 6.3.81 अव्ययीभावे चाकाले (saha→sa → सचक्रम्), 2.4.84 तृतीयासप्तम्योर्बहुलम् (optional fork → उपकृष्णम्/उपकृष्णे). **S4 corrections:** 1.1.41/2.4.18 moved to the samāsa pre-pass (`bahiranga: -1`); 2.1.12/2.1.13 now require + consume a kāraka-assigned pañcamī (?viBakti_5 → ?swap_viBakti → `_swap_sups`); 2.4.83 त्वपञ्चम्याः sub-rule (2.4.83.1 → ablative उपकृष्णात्). Derives अधिहरि, उपकृष्णम्, यथाशक्ति, यावज्जीवम्, अतिहिमम्, शाकप्रति, अक्षपरि, अपग्रामम्, आसमुद्रम्, प्रत्यग्नि, अनुवनम्, अनुगङ्गम्, द्विमुनि, द्विगङ्गम्, उपशरदम्, सचक्रम्, उपकृष्णम्/उपकृष्णे (18 cases in `test/samasa_list.py` / `test_samasa_avyayibhava.py`). **Deferred:** 2.1.17 (तिष्ठद्गु gaṇa), 2.1.18 (पारे/मध्ये irregular), 2.1.21 (अन्यपदार्थे saṁjñā); the सुमद्रम्/उन्मत्तगङ्गम् nitya-am exception under 2.4.84; samāsānta beyond TaC; `saptan` n-loss in compounds.

**Next to be implemented:** tatpuruṣa **Phase T1** — the remaining five vibhakti-tatpuruṣas (tṛtīyā 2.1.30–35, caturthī 2.1.36, pañcamī 2.1.37–39, **ṣaṣṭhī** 2.2.8 राजपुरुषः, saptamī 2.1.40–48), each the same shape as T0's dvitīyā (see `tatpuruza_plan.md` for the T0–T5 + T-liṅga + T-UI phasing and per-phase session prompts). Then karmadhāraya/dvigu (T2), nañ (T3), prādi/gati (T4), samāsānta (T5). bahuvrīhi / dvandva and the physical upasarjana pūrva-nipāta (2.2.30, carrying each member's sup) land when a non-pūrva-upasarjana type needs reorder. Kāraka deferrals (see Skipped table): the tiṅ sup-insertion branch, real abhidhāna detection, multi-clause/participle sentences, long-tail vārttikas. Strī/taddhita thread: the SK 532-numbered taddhita section (4.1.76 तद्धिताः adhikāra), with SK 508/509/516 and the वुग्युटावुवङ्यणोः vārttika deferred.
