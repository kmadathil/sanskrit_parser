...
**Last implemented:** the **entire kāraka-prakaraṇam** is complete — rule phases K0–K7 (SK 532–646: the kāraka saṁjñās 1.4.23–98 + the vibhakti chapter 2.3.1–73, all as `bahiranga: -1` pre-pass tag rules) plus the optional **K-UI** Vākya Composer (`generator/ui/app.py`, presentation only). Most recent work: the **kāraka review fixes (2026-06-27)** detailed below.

**Kāraka review fixes (2026-06-27):** a correctness/generality pass over the K5–K7 vibhakti rules, from a manual review.
**(1) Directional yoga-words (the headline fix).** Every yoga-word peek rule was written with TWO arms (`llp` OR `rrp`), so a yoga-word fired on *both* neighbours — `कृष्ण अन्य राम` wrongly gave `कृष्णाद् … रामाद्` (both pañcamī). Introduced a generic **user-chosen direction** `yoga_pUrva` (yoga-word governs the PRECEDING noun → it is the noun's `rrp`) / `yoga_para` (governs the FOLLOWING noun → it is the noun's `llp`), mirroring the K2 `kp_pUrva`/`kp_para` mechanism. Each peek rule now ANDs the direction tag into its arm, so one chosen direction fires exactly one arm: `कृष्ण अन्य(para) राम` → only `रामात्`. Applied to **2.3.4, 2.3.16, 2.3.19, 2.3.26, 2.3.27, 2.3.29, 2.3.30, 2.3.31(+.1), 2.3.32(+.1/.2), 2.3.34(+.1), 2.3.39, 2.3.40, 2.3.43, 2.3.44, 2.3.64**. The kṛd-yoga peeks (2.3.65/67/68/71) are deliberately left non-directional (the governed noun is picked by its own kāraka-sense tag, not adjacency; 2.3.65 governs both kartṛ and karman). Direction is set by `dir: pUrva|para` in the test composer and by a `pUrva`/`para` token on `-k`/`-w` in cmd_line. `yoga_pUrva`/`yoga_para` added to `_is_karaka_tag` (stripped off realized padas).
**(2) Engine enabler.** `process_yaml._exec_cond` and `sutra.evalConditionDetail` now support **nested condition groups** — a list element may itself be `[and,…]`/`[or,…]` (recurse), so `[and, [or, =anya, =ArAt], "?yoga_pUrva"]` is expressible. Backward compatible.
**(3) Generalized literal peeks to tags.** 2.3.30 `=dakziRatas`→`?atasuCarTa`, 2.3.31 `=dakziRena`→`?enap`, 2.3.64 `=paYcakftvas`→`?kftvasuCarTa`, 2.3.26/27 `=hetoH/=hetunA`→`?hetu`; 2.3.29 keeps literals for anya/ārāt/itara/ṛte and adds `?dikSabda` for the dik-word class (replacing the dead `=pUrva`). New avyaya `uttaratas`/`uttareRa`/`saptakftvas` and dik stems `pUrva_dik`/`uttara_dik` + adjectives `prasita`/`utsuka` demonstrate generality.
**(4) Split conflated verb tags.** 2.3.52 `?aDIgartha`→`?aDIga`/`?daya`/`?IS` (+ verbs `dayate`/`Izwe`); 2.3.56 now `[and, ?jasAdi, ?hiMsArTa]` (+ verb `pinazwi`); 2.3.57 `?vyavahfpaR`→`?vyavahf`/`?paR` (+ verb `paRate`); 2.3.58/59 `?divtadarTa`→`?div`+`?paRArTa`; 2.3.61 `?prezyabrU`→`[and, [or ?prez ?brU], ?devatAsaMpradAna]` with a `?semantic_havis` guard on the karman (+ verb `brUte`).
**(5) Targeted corrections.** 2.3.24 (akartari ṛṇe): added the missing non-agency guard `?!semantic_svatantra` + `?!semantic_prayojaka` (the comment had claimed a `?!kAraka` guard that was absent); a debt that IS the agent no longer gets pañcamī. 2.3.37 (bhāvalakṣaṇa sati-saptamī): dropped the partner-peek, now lp-only (`?semantic_BAvalakzaRa`) — no adjacency requirement. 2.3.44 (prasita/utsuka): was incorrectly tagging the object itself; now a directional peek of a separate `?prasitotsuka` adjective word governing the `kAraka_aDikaraRa` object (tṛtīyā/saptamī fork). **157 kāraka tests pass (+13); the full generator suite (8022/8022) passes — the nested-condition-group matcher change introduced no regressions.**

**Samāsa-prakaraṇam — avyayībhāva (SK 647–677), DONE** (avyayībhāva samāsa plan, S0–S3). A new **samāsa pre-pass** (`AntarangaPrakriya._samasa_prepass`, `bahiranga: -1`) runs after the kāraka pre-pass + sup-insertion (सह सुपा 2.1.4): the 113 kāraka rules were renumbered `bahiranga: -1 → -2` to make room (lower = more antaraṅga = earlier; kāraka -2 fixes vibhakti, samāsa -1 consumes it). The pre-pass scans adjacent (pūrva | uttara) member windows and assigns the samāsa saṁjñā / member-role (`samAsaPurva`/`samAsa`) / `avyayIBAva` / `upasarjana` (1.2.43) tags; it does NOT luk internal vibhakti (main scan) or physically reorder (2.2.30 deferred — the avyaya is already pūrva). Main-scan morphology: 1.1.41 (avyaya on the uttara), 2.4.71 (pūrva sup-luk), 2.4.82 (luk) / 2.4.83 (a-stem → अम्, overrides 2.4.82), 2.4.18 (napuṁsaka) + 1.2.47 (hrasva). Sutras: **nitya** 2.1.5/2.1.6 (समीप/विभक्ति/अत्यय/…), 2.1.7 (यथा), 2.1.8 (यावत्), 2.1.9 (सुप्प्रति, noun-pūrva), 2.1.10 (अक्षशलाका, noun-pūrva); **vibhāṣā** (≥ SK665, gated by `?samAsa_vivakza` — "only when intended", not a fork) 2.1.12/2.1.13/2.1.14 (प्रत्यग्नि)/2.1.15/2.1.16/2.1.19/2.1.20; **samāsānta/junction/bahula** 5.4.107 TaC (उपशरदम्, via `_insert_samasanta`), 6.3.81 अव्ययीभावे चाकाले (saha→sa → सचक्रम्), 2.4.84 तृतीयासप्तम्योर्बहुलम् (optional fork → उपकृष्णम्/उपकृष्णे). Derives अधिहरि, उपकृष्णम्, यथाशक्ति, यावज्जीवम्, अतिहिमम्, शाकप्रति, अक्षपरि, अपग्रामम्, आसमुद्रम्, प्रत्यग्नि, अनुवनम्, अनुगङ्गम्, द्विमुनि, द्विगङ्गम्, उपशरदम्, सचक्रम्, उपकृष्णम्/उपकृष्णे (18 cases in `test/samasa_list.py` / `test_samasa_avyayibhava.py`). **Deferred:** 2.1.17 (तिष्ठद्गु gaṇa), 2.1.18 (पारे/मध्ये irregular), 2.1.21 (अन्यपदार्थे saṁjñā); the सुमद्रम्/उन्मत्तगङ्गम् nitya-am exception under 2.4.84; samāsānta beyond TaC; `saptan` n-loss in compounds.

**Next to be implemented:** the remaining samāsa types (tatpuruṣa / bahuvrīhi / dvandva) reuse the S0 shared spine (the samāsa pre-pass, upasarjana/eka-vibhakti, sup-luk); physical upasarjana pūrva-nipāta (2.2.30, carrying each member's sup) lands when a non-avyaya-pūrva type needs reorder. Kāraka deferrals (see Skipped table): the tiṅ sup-insertion branch, real abhidhāna detection, multi-clause/participle sentences, long-tail vārttikas. Strī/taddhita thread: the SK 532-numbered taddhita section (4.1.76 तद्धिताः adhikāra), with SK 508/509/516 and the वुग्युटावुवङ्यणोः vārttika deferred.
