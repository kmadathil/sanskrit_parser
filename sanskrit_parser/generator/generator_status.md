...
**Last implemented — Samāsa-prakaraṇam: tatpuruṣa (SK 684–, Phase T0)** (tatpuruṣa samāsa plan `tatpuruza_plan.md`, Phase T0 — dvitīyā-tatpuruṣa proof-of-concept). Unlike the avyayībhāva (which becomes an indeclinable → invariant अम्), a **tatpuruṣa declines normally in the uttara's gender** (2.4.26 परवल्लिङ्गम्): the uttara keeps its own sup and inflects, while the pūrva (the case-marked upasarjana) sup-luks via the existing 2.4.71 — so we do NOT tag `?avyaya`/`?napum` on the uttara. Gender inheritance is already realized by `join_objects` (the compound merge prefers the last member's gender, `paninian_object.py` ~L154), so **2.4.26 is a documenting saṁjñā** (`?paravalliNga` marker) that the T-liṅga exceptions will override. New rules (`bahiranga: -1`, reusing the samāsa pre-pass spine): **2.1.24** द्वितीया श्रितातीत… (fused with the 2.1.22 तत्पुरुषः saṁjñā, as 2.1.5 was fused into 2.1.6) — a dvitīyā noun (`?viBakti_2`) + a śrita-gaṇa uttara (`?srita_gaRa`: श्रित/अतीत/…) → pūrva `?samAsaPurva`, uttara `?samAsa`+`?tatpuruza`; gated on `?samAsa_vivakza` (only-when-intended); **2.4.26** परवल्लिङ्गम्. Reused unchanged: 1.2.43 (upasarjana), 2.4.71 (pūrva sup-luk). Derives **कृष्णश्रितः** and its full a-stem masc vibhakti sweep (कृष्णश्रितम्/कृष्णश्रितेन/…/कृष्णश्रितात् — proving it inflects, unlike the avyayībhāva's invariant अम्). New pratipadika `Srita` (`?srita_gaRa`). 9 cases in `test/test_samasa_tatpurusha.py` (`samasa_list.py::samasa_tp_tests` + the sweep). **Deferred to later T-phases:** the other five vibhakti-tatpuruṣas (T1), karmadhāraya/dvigu (T2), nañ (T3), prādi/gati (T4), samāsānta (T5), gender exceptions (T-liṅga), composer/CLI (T-UI); 2.2.30 physical reorder stays deferred (the case member is already pūrva).
